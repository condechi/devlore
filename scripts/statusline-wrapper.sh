...
#
# Multi-KB dispatch: KB roots are listed in ~/.devlore/kb-dirs (one per line,
# # comments; written by `devlore init`; pre-v0.9.23 installs used
# ~/.claude/kb-dirs — read as a fallback until any devlore command migrates it).
# Each KB's statusline.py is GATED — it
...
kbs=()
REG="$HOME/.devlore/kb-dirs"
[ -f "$REG" ] || REG="$HOME/.claude/kb-dirs"
if [ -f "$REG" ]; then
